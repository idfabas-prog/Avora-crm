...
test("formats source panel details without exposing location IDs", () => {
  const trace = { tools: ["getOwnerAnalytics"], locations: ["uuid-1"], recordCounts: { contacts: 4, completed_appointments: 0 } };
  assert.equal(describeLocationScope(trace), "Selected location");
  assert.deepEqual(sourceRows(trace), ["0 Completed Appointments", "4 Contacts"]);
});
