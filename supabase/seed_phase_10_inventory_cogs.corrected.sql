...
select
  ...
  (select count(*) from public.vendor_items vi join public.vendors v on v.id = vi.vendor_id join public.organizations o on o.id = v.organization_id where lower(trim(o.slug)) = 'avora') as vendor_items,
  (select count(*) from public.inventory_location_settings ils join public.organizations o on o.id = ils.organization_id where lower(trim(o.slug)) = 'avora') as inventory_location_settings,
  ...
  (select count(*) from public.purchase_order_items poi join public.purchase_orders po on po.id = poi.purchase_order_id join public.organizations o on o.id = po.organization_id where lower(trim(o.slug)) = 'avora') as purchase_order_items,
  ...
  (select count(*) from public.inventory_service_requirements isr join public.organizations o on o.id = isr.organization_id where lower(trim(o.slug)) = 'avora') as inventory_service_requirements,
  (select count(*) from public.inventory_transfers it join public.organizations o on o.id = it.organization_id where lower(trim(o.slug)) = 'avora') as inventory_transfers,
  (select count(*) from public.inventory_transfer_items iti join public.inventory_transfers it on it.id = iti.transfer_id join public.organizations o on o.id = it.organization_id where lower(trim(o.slug)) = 'avora') as inventory_transfer_items,
  (select count(*) from public.inventory_alerts ia join public.organizations o on o.id = ia.organization_id where lower(trim(o.slug)) = 'avora') as inventory_alerts,
  (select count(*) from public.workflows w join public.organizations o on o.id = w.organization_id where lower(trim(o.slug)) = 'avora' and w.category = 'inventory') as inventory_workflows;
